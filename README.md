...
В рамках данной домашней работы мы займемся более инфраструктурными вопросами, а именно подключением пары полезных плагинов и доработкой пайплайна в GitHub Actions.

### Плагины

Необходимо будет подключить и настроить несколько полезных плагинов в ваших проектах:

* Apache Maven Checkstyle Plugin - https://maven.apache.org/plugins/maven-checkstyle-plugin/. Подробнее можно почитать тут. Пока что будет достаточно и дефолтной конфигурации. Соответственно, ваш проект необходимо будет привести к данном виду.
* Jacoco Maven Plugin - https://github.com/jacoco/jacoco. Необходим для отслеживания покрытия кода ваших проектов юнитами. Нужно добиться, чтобы оно (покрытие) было не ниже 50%.
### Пайплайн
На втором шаге необходимо будет настроить пайплайн для ваших сервисов. Для каждого сервиса (отдельного модуля), пайплайн должен выглядеть следующим образом:

#### 1 stage: build
На данном шаге выполняется команда mvn clean compile для проверки, что ваш код вообще компилируется.

#### 2 stage: checkstyle
На данном шаге выполняется команда mvn checkstyle:check для проверки, что ваш код соответствует чек-сатйлу.

#### 3 stage: unit tests
На данном шаге выполняется команда mvn clean install для запуска юнит-тестов. Помимо этого, с помощью экшена madrapps/jacoco-report в комментарии к связанному МРу необходимо выгрузить информацию о покрытии тестами вашего исходного кода.

#### 4 stage: deploy
Так как деплоить нам особо некуда, вместо этого предлагается после мержа ветки собирать новый образ и пушить его в DockerHub (подробнее тут). Так как в Dockerhub есть ограничение на кол-во образов в бесплатной версии, вам придется воспользоваться механизмом тэгов (подробнее тут).
